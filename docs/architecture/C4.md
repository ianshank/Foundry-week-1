...
2. **Integration Layer (`tests/integration/`)**: FastMCP server protocol dispatch, stdio transport, and live parameter validation.
...
graph TD
    ...
    guards["<b>guards.py</b><br/><b>POLICY — hard-coded</b><br/>verb allow list · flag deny list<br/>path containment · secret patterns"]
    config["<b>config.py</b><br/><b>CONFIG — from env</b><br/>paths · timeouts · limits<br/>severity vocabulary"]
...
flowchart LR
    ...
    guard -->|"refused"| blocked["<b>BLOCKED</b><br/>blocked_reason"]
    ...
    payload["stdout unparsable?"] -.->|"sets findings_parse_error,<br/><b>never changes the verdict</b>"| code
